/* Répétition de motifs (pour l'atelier Tricot Machine à Carhaix)
 ...
 version 005 : nouveau cahier des charges!
   * définition fixée à 1024 x 768 (pour utiliser sur vieux moniteurs)
   * damier porté à 64 x 64
 */
...
int elements_x = 64;         // nombre de colonnes composant le motif
int elements_y = 64;         // nombre de rangées composant le motif
int decalage_x = 16;         // décalage de la zone active du motif
int decalage_y = 16;
...
String repertoire = "";      // répertoire dans lequel seront enregistrés les motifs


int motif_largeur, motif_hauteur;        // dimensions maximales du motif 
int motif_taille = 8;                    // taille du carré représentant un pixel dans l'éditeur de motif
int xs = 30;                             // coordonnée horizontale du début de l'affichage du motif
int ys = 85;                             // coordonnée verticale du début de l'affichage du motif

int repetition_echelle = 1;              // échelle d'agrandissement du motif répété
int decalage_repetition_x = 560;         // coordonnée horizontale du début de l'affichage du motif répété   
...
void setup() {
  ...
  motif_image = createGraphics(motif_largeur, motif_hauteur);
  
  // L'appli s'ouvre dans l'état ou on l'a laissée
  brouillon = createGraphics(elements_y, elements_x);
...
void draw() {
  ...
  fill(255);
  text(nom_fichier_brut, xs + 20, 705);
  ...
  fill(255);
  text(brouillon_travail, xs + 20, 720);
  ...
  if (quelquechose_change) {
    ...
    motif_image = createGraphics(motif_largeur*repetition_echelle, motif_hauteur*repetition_echelle);
...
void sauverMotif() {
  motif_image_brut = createGraphics(motif_largeur, motif_hauteur);
  motif_image_brut.beginDraw();
  motif_image_brut.noStroke();
  for (int y = decalage_y; y < elements_y; y++) {
    for (int x = decalage_x; x < elements_x; x++) {
      if (motif[y][x] == 0) {
        motif_image_brut.fill(0, 0, 0, 0);
      }
      if (motif[y][x] == 1) {
        motif_image_brut.fill(255,255,255,255);
      }
      if (motif[y][x] == 2) {
        motif_image_brut.fill(0, 0, 0, 255);
      }
      motif_image_brut.rect(x-decalage_x, y-decalage_y, 1, 1);
    }
  }
  motif_image_brut.endDraw();
  motif_image_brut.save(repertoire + "/" + nom_fichier_brut);
  fichier_enregistre_brut = true;
}

// Sauver le brouillon
void sauverBrouillon() {
  brouillon = createGraphics(elements_x, elements_y);
  brouillon.beginDraw();
  brouillon.noStroke();
  for (int y = 0; y < elements_y; y++) {
    for (int x = 0; x < elements_x; x++) {
...
void chargerImage(String fichier) {
  ...
  if (f.isFile()) {
    ...
    if (dimx > elements_x) dimx = elements_x;
    if (dimy > elements_y) dimy = elements_y;
